...
// .wrangler/tmp/bundle-Js1RbU/checked-fetch.js
...
async function fetchContentFromUrl(url) {
  try {
    ...
    if (error3 instanceof Error) {
      throw new Error(`Error fetching content: ${error3.message}`);
    }
    throw new Error("Unknown error occurred while fetching content.");
  }
}
__name(fetchContentFromUrl, "fetchContentFromUrl");
...
function createErrorResponse(code, message, moreInfo) {
...
function countTokens(text) {
  ...
  return Math.max(1, Math.ceil(tokens * 1.3));
}
__name(countTokens, "countTokens");

// src/utils/splitContent.ts
function splitContentIntoChunks(content, maxTokens, respectSentences = false) {
  if (!respectSentences) {
...
__name(splitContentIntoChunks, "splitContentIntoChunks");

// src/utils/generateSummary.ts
async function generateSummaryForChunk(chunk, options, apiKey, maxOutputTokens) {
...
__name(generateSummaryForChunk, "generateSummaryForChunk");
async function generateSummary(content, options, apiKey) {
  const maxTokenLimit = 16385;
  const maxOutputTokens = options.word_count || 100;
  const maxInputTokens = maxTokenLimit - maxOutputTokens;
  const cleanedContent = cleanContent(content);
  const contentTokens = countTokens(cleanedContent);
  const chunks = contentTokens <= maxInputTokens ? [cleanedContent] : splitContentIntoChunks(cleanedContent, maxInputTokens, true);
  const summaries = await Promise.all(
    chunks.map(
      (chunk) => generateSummaryForChunk(chunk, options, apiKey, maxOutputTokens)
    )
  );
...
var worker_default = {
  async fetch(request, env3) {
    try {
      ...
      const { url, summary_options } = requestData;
      ...
      const summary = await generateSummary(content, options, env3.OPENAI_API_KEY);
      return new Response(JSON.stringify({ summary }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
    } catch (error3) {
      const errorMessage = error3 instanceof Error ? error3.message : "An unknown error occurred.";
...
// .wrangler/tmp/bundle-Js1RbU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Js1RbU/middleware-loader.entry.ts
